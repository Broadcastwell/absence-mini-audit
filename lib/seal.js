...
/** The exact fields a seal covers, in a fixed order. */
export function canonical(brand, category, company, result) {
  const questions = Array.isArray(result && result.questions) ? result.questions.map((row) => [String(row.question), String(row.status)]) : [];
  return JSON.stringify(["v1", String(brand || ""), String(category || ""), String(company || ""), result.named, result.asked, String(result.tier), String(result.chapter), String(result.engine), String(result.measured_on), questions]);
}
