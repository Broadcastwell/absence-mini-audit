 ...
 *
 * A link is revocable. POST /api/link also returns a delete token (128 random bits) that
 * only the browser which asked for the link receives; the record keeps a SHA-256 of it and
 * never the token. POST /api/unlink with the id and that token deletes the record at once,
 * after which the link answers with the expired state.
 */

import { sealKey, verify, cleanBrand } from "./seal.js";
import { TIERS, CHAPTERS, rowExtras } from "./audit.js";
...
/**
 * Keep only the seven public keys, in their public shapes, and on each row only its question,
 * its status and the cleaned receipt fields. Anything else is refused.
 */
export function cleanResult(result) {
  ...
  const rows = questions.map((row) => Object.assign({ question: String((row && row.question) || "").slice(0, 500), status: String((row && row.status) || "") }, rowExtras(row)));
...
export async function link(request, env, options) {
  ...
  const id = newId();
  const token = newId();
  const record = { v: 1, id: id, brand: brand, category: category, website: company, result: result, created_on: today, expires_on: day(LINK_TTL_DAYS), revoke_hash: await sha256Hex(token) };
  await env.AUDIT.put("link:" + id, JSON.stringify(record), { expirationTtl: LINK_TTL_DAYS * 86400 });
  return json({ id: id, url: new URL("/r/" + id, request.url).toString(), expires_on: record.expires_on, reference: "fc_" + id, revoke_token: token }, 200);
}
...
/**
 * Delete a result link before its 30 days are up. Only the delete token returned when the
 * link was made can do it. A link that is already gone answers as deleted, so the answer
 * says nothing about whether an id ever existed. A link made before tokens existed cannot be
 * deleted here; the answer says to email for it.
 */
export async function unlink(request, env) {
  let payload;
  try { payload = await request.json(); } catch (_) { return json({ message: "The link could not be deleted.", reason: "input" }, 400); }
  const id = String((payload && payload.id) || "");
  const token = String((payload && payload.token) || "");
  if (!/^[0-9a-f]{32}$/.test(id) || !/^[0-9a-f]{32}$/.test(token)) return json({ message: "The link could not be deleted.", reason: "input" }, 400);
  if (!env || !env.AUDIT) return json({ message: "Result links are not available right now.", reason: "unavailable" }, 503);
  let record = null;
  try { record = await env.AUDIT.get("link:" + id, "json"); } catch (_) { record = null; }
  if (!record) return json({ deleted: true }, 200);
  if (!sameText(await sha256Hex(token), record.revoke_hash)) {
    return json({ message: "This browser cannot delete that link. Email hello@broadcastwell.com and we will delete it.", reason: "token" }, 403);
  }
  await env.AUDIT.delete("link:" + id);
