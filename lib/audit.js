 ...
 * redeploy.
 *
 * Pages requires atomic D1 reservations. Failed or ambiguous provider calls
 * retain their allowance because they may already have incurred usage. The
 * legacy worker compatibility path below retains its old KV behavior; it is
 * not reachable through the guarded Pages entry point without atomic storage.
 */

import {reserveFreeCheck} from './reservations.js';
import {beginCost,finishCost,reportedCost} from './costs.js';
...
function questionResults(data, asked) {
  ...
  const normalized = source.map((item) => {
    ...
    if(item?.answer!==undefined||item?.answer_full!==undefined){
      const answer=item.answer??item.answer_full;
      if(typeof answer!=='string'||answer.length>20000)return null;
      result.answer=answer;
    }
    if(item?.alternatives!==undefined){
      if(!Array.isArray(item.alternatives)||item.alternatives.length>100)return null;
      const alternatives=item.alternatives.map(value=>typeof value==='string'?value.trim():typeof value?.name==='string'?value.name.trim():null);
      if(alternatives.some(value=>!value||value.length>250))return null;
      result.alternatives=alternatives;
    }
    if(item?.sources!==undefined||item?.cited_urls!==undefined){
      const sources=item.sources??item.cited_urls;
      if(!Array.isArray(sources)||sources.length>200)return null;
      result.sources=[];
      for(const entry of sources){
        const value=typeof entry==='string'?entry:entry?.url;
        try{const url=new URL(value);if(url.protocol!=='https:'||url.username||url.password||value.length>4000)continue;result.sources.push(url.href);}catch{}
      }
      result.source_urls_omitted=sources.length-result.sources.length;
    }
    return result;
  });
...
export async function run(request, env) {
  ...
  const cfg = env.LIMITS_DB ? null : await limits(env);
  const day = today();
  ...
  const reserved = [];
  let costId=null,costValue=null;
  ...
  async function noResult(reason, detail) {
    if(costId)await finishCost(env.LIMITS_DB,costId,costValue,'failed');
    if(!env.LIMITS_DB)for (const key of reserved.slice().reverse()) await release(env, key, ttl);
    console.warn(JSON.stringify(Object.assign({ event: "mini_audit_no_result", reason: reason, released: reserved.length }, detail || {})));
    return json({ message: env.LIMITS_DB?'We could not complete this check. Its daily allowance is retained because the request may have been processed. Please try again tomorrow.':RESPONSES.upstream }, 502);
  }
  ...
  if(env.LIMITS_DB){
    let reservation;
    try{reservation=await reserveFreeCheck(env.LIMITS_DB,{email,ip,day});}
    catch{return json({message:'This check is temporarily unavailable. Please try again later.'},503);}
    if(!reservation.allowed)return json({message:RESPONSES[reservation.reason]},reservation.reason==='global_limit'?503:429);
    try{await beginCost(env.LIMITS_DB,reservation.id);costId=reservation.id;}catch{return json({message:'This check is temporarily unavailable. Please try again later.'},503);}
  }else{
  // Compatibility for the legacy worker contract; Pages requires LIMITS_DB.
  ...
  if (!upstream.ok) {
    try{costValue=await upstream.json();}catch{}
  ...
  const questions=questionResults(data,asked);
  const cost=reportedCost(data);
  if(env.LIMITS_DB&&(!cost||cost.actual_usd===null||questions.length!==10||questions.some(q=>!q.answer?.trim()||!Array.isArray(q.alternatives)||!Array.isArray(q.sources))))return noResult('incomplete_evidence_or_cost',{ms:Date.now()-started});
  if(costId)await finishCost(env.LIMITS_DB,costId,data,'complete');
  return json({
    named: named,
    asked: asked,
    tier: tier,
