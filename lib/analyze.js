...
export const ANALYZE_DEFAULTS = { analyze_per_ip_per_day: 20, analyze_global_per_day: 300 };
  ...
  const scored = LEXICON.map((entry) => {
    ...
    for (const term of entry.terms) {
      ...
      for (const [text, weight] of lowered) {
        if (!text) continue;
        const hits = countMatches(text, term);
        if (hits) score += weight * multiplier * Math.min(hits, weight < 1 ? 6 : 3);
      }
    }
    const label = entry.label.toLowerCase();
    for (const [text, weight] of lowered) if (text && countMatches(text, label)) score += weight * 3;
